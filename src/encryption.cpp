...
public:
    ...
    EncryptedData encrypt(const std::vector<uint8_t> &plaintext)
    {
        ...
        std::vector<uint8_t> associatedData = generateRandomBytes(16);
...
private:
    ...
    static void chachaCore(uint32_t state[16])
    {
        for (int i = 0; i < 10; ++i)
        {
    ...
    static std::vector<uint8_t> chacha20(const std::vector<uint8_t> &key, const std::vector<uint8_t> &nonce, const std::vector<uint8_t> &plaintext, uint32_t counter = 0)
    {
        if (key.size() != 32)
            throw std::runtime_error("Key must be 256 bits long.");
    ...
    static EncryptedData encryptChaCha20Poly1305(const std::vector<uint8_t> &key, const std::vector<uint8_t> &nonce, const std::vector<uint8_t> &plaintext, const std::vector<uint8_t> &associatedData)
    {
        ...
        auto *adPadded = new uint8_t[adPaddedSize];
        ...
        auto *ctPadded = new uint8_t[ctPaddedSize];
    ...
    static bool decryptChaCha20Poly1305(const std::vector<uint8_t> &key, const std::vector<uint8_t> &nonce, const EncryptedData &encryptedData, const std::vector<uint8_t> &associatedData, std::vector<uint8_t> &decryptedText)
    {
        std::vector<uint8_t> subkey = chacha20(key, nonce, std::vector<uint8_t>(32), 0);
...
std::vector<uint8_t> SilkCasket::encryptFile(const std::string &keyStr, const std::vector<uint8_t> &plaintext) {
    try
    {
...
std::vector<uint8_t> SilkCasket::decryptFile(const std::string &keyStr, const std::vector<uint8_t> &inputData) {
