...
std::vector<uint8_t> SilkCasket::Compress::LZW::decompress(const vector<uint8_t> &compressed) {
    ...
    std::vector<uint8_t> result;  // 存储解压缩后的数据
    size_t index = 0;             // 当前读取位置的索引

    // 解码第一个变长编码值
    uint32_t old = decodeVLE(compressed, index);
    result.push_back(static_cast<uint8_t>(old));  // 将解码后的值添加到结果中

    // 遍历压缩数据的每个变长编码值
    while (index < compressed.size()) {
        uint32_t entry = decodeVLE(compressed, index);  // 解码当前变长编码值
